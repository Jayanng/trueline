...
| 2:00–2:25 | Green PR twin (`demo/pr-safe-add`) | “Same pipeline, additive-only notes → LOW. The gate isn’t always red.” |
| 2:25–2:45 | Optional: `--commit --verify` or skill | “After merge, Trueline writes missing column lineage — the graph gets truer.” |
| 2:45–2:55 | Architecture one-liner | “MCP + GMS for facts; LLM only for prose. Severity is deterministic.” |
